...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderPredictions(state) {
    ...
    console.log('[DTE] renderPredictions: weeklyH='+weeklyH+' hs='+hs+' restDays='+JSON.stringify(_rdNow)+' _f='+(freshState.scores&&freshState.scores._f));
    try { sim  = DTE.simulator.run({ days, hoursPerDay: hs, restDays: _rdNow }, freshState.scores); } catch(e) { console.warn('[DTE] sim error:', e); }
    try { fut  = DTE.simulator.futurState(days, freshState.norm); } catch(e) { console.warn('[DTE] fut error:', e); }
    try { scen = DTE.simulator.scenarios(days, freshState.norm); } catch(e) { console.warn('[DTE] scen error:', e); }
    if (sim) console.log('[DTE] sim.summary:', JSON.stringify(sim.summary).slice(0,200));
