...
function updateAllDisplays() {
    try {
        ...
        console.warn('updateAllDisplays error:', e);
    }
}
...
function updatePlayerStats() {
    ...
    if (xpEl && typeof xpSystem !== 'undefined') xpEl.textContent = xpSystem.currentXP;
    if (nextXpEl && typeof xpSystem !== 'undefined') nextXpEl.textContent = xpSystem.getXPForNextLevel(gameState.player.level);
    
    if (progressEl && typeof xpSystem !== 'undefined') {
...
function updateLeague() {
    if (typeof xpSystem === 'undefined' || typeof leagueSystem === 'undefined') return;
    const league = leagueSystem.getCurrentLeague(xpSystem.currentXP);
    const leagueEl = document.getElementById('league-display');
    if (leagueEl && league) leagueEl.textContent = league.name;
}
...
function displayModule1() {
    ...
    if (!container) return; // Élément absent
...
function displayModule2() {
    ...
    container.innerHTML = `
        <div class="module-summary">
            ...
            <div class="stat-row">
                <span>Heures totales:</span>
                <strong>${m2?.totalHours ? m2.totalHours.toFixed(1) : 0}h</strong>
            </div>
            <div class="contingent-bar">
                <div class="contingent-label">Contingent: ${m2?.contingent?.used ? m2.contingent.used.toFixed(1) : 0}h / ${m2?.contingent?.total || 220}h</div>
                ...
                <div class="contingent-remaining">${m2?.contingent?.remaining ? m2.contingent.remaining.toFixed(1) : 0}h restantes</div>
            </div>
            <div class="stat-row">
