  ...
  getCumulatedHours() {
    ...
    years.forEach(y => {
      ...
      Object.entries(m1.monthlyBreakdown || {}).forEach(function(entry) {
        var mk = entry[0], v = entry[1];
        if (!m1Months[mk]) m1Months[mk] = { extra: 0, recup: 0, absent: 0, overtime: 0, hs25: 0, hs50: 0 };
        m1Months[mk].extra   += v.extra    || 0;
        m1Months[mk].recup   += v.recup    || 0;
        m1Months[mk].absent  += v.absent   || 0;
        m1Months[mk].overtime += v.overtime || 0;
        m1Months[mk].hs25    += v.hs25     || 0;
        m1Months[mk].hs50    += v.hs50     || 0;
      });
      ...
      allMonths.forEach(m => {
        ...
        if (src === 'M1') {
          ...
          const hs25  = d.hs25 != null ? d.hs25 : Math.min(8, hsNet);
          const hs50  = d.hs50 != null ? d.hs50 : Math.max(0, hsNet - 8);
          yearExtra   += hsNet;
          yearPlus25  += hs25;
          yearPlus50  += hs50;
          yearAbsent  += d.absent || 0;
          monthDetail[m] = { src: 'M1', extra: hsNet, hs25, hs50 };
        } else {
          const d = m2Months[m];
          yearPlus25 += d.plus25;
          yearPlus50 += d.plus50;
