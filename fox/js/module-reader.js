...
class ModuleReaderPro extends ModuleReader {
  ...
  loadModule1ForYear(year) {
    try {
      ...
      console.log(
        'M1[' + year + '] rawData=' + Object.keys(rawData).length + 'j',
        '| semaines=' + Object.keys(weeklyData).length,
        '| netOvertime=' + netOvertime.toFixed(2) + 'h',
        '| mois=', Object.keys(monthlyBreakdown)
      );
      return {
  ...
  loadModule2ForYear(year) {
    try {
      ...
      console.log(
        'M2[' + year + '] hasDays=' + hasDays + ' hasOldFormat=' + hasOldFormat,
        '| totalAnnual=' + totalAnnual.toFixed(2) + 'h',
        '| hs25=' + totalPlus25.toFixed(2) + 'h',
        '| hs50=' + totalPlus50.toFixed(2) + 'h',
        '| mois=', Object.keys(monthlyBreakdown)
      );
  ...
  getCumulatedHours() {
    ...
    years.forEach(y => {
      ...
      // m1Raw supprime - on utilise m1.monthlyBreakdown (calcule par loadModule1ForYear)
      ...
      // M1 : utiliser monthlyBreakdown deja calcule par loadModule1ForYear
      // (evite de relire rawData avec les mauvais champs)
      Object.entries(m1.monthlyBreakdown || {}).forEach(function(entry) {
        var mk = entry[0], v = entry[1];
        if (!m1Months[mk]) m1Months[mk] = { extra: 0, recup: 0, absent: 0 };
        m1Months[mk].extra   += v.extra   || 0;
        m1Months[mk].recup   += v.recup   || 0;
        m1Months[mk].absent  += v.absent  || 0;
        m1Months[mk].overtime = (m1Months[mk].overtime || 0) + (v.overtime || 0);
      });
      ...
      allMonths.forEach(m => {
        ...
        if (src === 'M1') {
          const d = m1Months[m];
          // Utiliser overtime (HS nettes reelles) si disponible, sinon extra-recup
          const hsNet = d.overtime != null ? d.overtime : Math.max(0, d.extra - d.recup - d.absent);
          yearExtra  += hsNet;
          yearRecup  += 0; // deja deduit dans overtime
          yearAbsent += d.absent || 0;
          monthDetail[m] = { src: 'M1', extra: hsNet };
        } else {
          const d = m2Months[m];
          yearPlus25 += d.plus25;
          yearPlus50 += d.plus50;
    ...
    console.log(
      ...
      '| annees=', years,
      '| netOvertime=' + netOvertime.toFixed(2) + 'h',
      '| totalPlus25=' + totalPlus25.toFixed(2) + 'h',
      '| totalPlus50=' + totalPlus50.toFixed(2) + 'h',
