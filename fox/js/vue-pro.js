...
(function () {
  ...
  function fmtH(h) {
    ...
    var hh = Math.floor(t); var mm = Math.round((t - hh) * 60);
  ...
  function calcAnnual(yr) {
    var total = 0; var src = 'aucune'; var hasM1 = false;
    var m1 = readM1(yr);
    Object.keys(m1).forEach(function(k) { if (/^\d{4}-\d{2}-\d{2}$/.test(k) && (m1[k].extra||0) > 0) { total += m1[k].extra; hasM1 = true; } });
    if (hasM1) return { total: Math.round(total*10)/10, src: 'M1' };
    var m2 = readM2(yr);
    Object.keys(m2).forEach(function(mk) { var mv = m2[mk]; if (mv && mv.days) Object.keys(mv.days).forEach(function(d) { total += (mv.days[d]||0); src = 'M2'; }); });
    return { total: Math.round(total*10)/10, src: src };
  }
  ...
  function calcToday(yr) {
    ...
    if (m2[mk] && m2[mk].days && m2[mk].days[d]) return { extra: m2[mk].days[d], total: 0 };
  ...
  function getMonthly(yr) {
    ...
    return mo.map(function(v){ return Math.round(v*10)/10; });
  }
  ...
  function adviceCard(item) {
    ...
    return '<div style="background:' + bg + ';border:1px solid rgba(255,255,255,0.05);border-left:3px solid ' + bc + ';border-radius:0 9px 9px 0;padding:12px 13px;margin-bottom:8px;">'
      ...
      + (item.loi ? '<div style="margin-top:5px;"><span style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:4px;padding:2px 6px;font-size:0.6rem;color:#37474F;letter-spacing:0.5px;">' + item.loi + '</span></div>' : '')
      + '</div></div></div>';
  }
  ...
  function render() {
    ...
    var html = ''
      ...
      // ══ HEADER ════════════════════════════════════════════════
      + '<div style="position:sticky;top:0;z-index:5;background:rgba(6,8,15,0.97);backdrop-filter:blur(8px);border-bottom:1px solid rgba(255,255,255,0.05);padding:13px 18px;display:flex;align-items:center;justify-content:space-between;">'
      ...
      + section('Références légales',
          ...
          + [
            ...
            var url = 'https://www.legifrance.gouv.fr/codes/article_lc/LEGI' + r[0].replace('L','ARTI000');
            var legifrance = 'https://www.legifrance.gouv.fr/search/code?tab_selection=code&query=' + r[0] + '&searchField=ALL&code=LEGITEXT000006072050&dateVersion=&etatArticle=VIGUEUR&etatTexte=&typePagination=DEFAULT&sortValue=&pageSize=10&page=1&tab_selection=code';
            return '<div style="display:flex;gap:10px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,0.03);align-items:center;">'
              + '<a href="' + legifrance + '" target="_blank" rel="noopener" style="font-size:0.65rem;color:#546E7A;min-width:72px;font-family:monospace;text-decoration:none;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:4px;padding:2px 6px;display:inline-block;transition:background 0.15s;" onmouseover="this.style.background=\'rgba(255,255,255,0.08)\'" onmouseout="this.style.background=\'rgba(255,255,255,0.04)\'">Art. ' + r[0] + ' ↗</a>'
      ...
      + section('Modules',
          ...
          + '<a href="../heures/index.html" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:9px;padding:13px;text-align:center;text-decoration:none;display:block;"><div style="font-size:1.3rem;margin-bottom:3px;">📅</div><div style="font-size:0.75rem;color:#546E7A;font-weight:600;">Suivi annuel</div><div style="font-size:0.62rem;color:#2D3F4A;margin-top:2px;">Module M1</div></a>'
          + '<a href="../paye/index.html" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:9px;padding:13px;text-align:center;text-decoration:none;display:block;"><div style="font-size:1.3rem;margin-bottom:3px;">💰</div><div style="font-size:0.75rem;color:#546E7A;font-weight:600;">Paie mensuelle</div><div style="font-size:0.62rem;color:#2D3F4A;margin-top:2px;">Module M2</div></a>'
          + '</div>'
          + '<button onclick="openPopup(\'popup-analyse\')" style="width:100%;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:9px;padding:12px;font-size:0.78rem;color:#455A64;cursor:pointer;font-family:inherit;margin-bottom:8px;">⚖️ Simuler une semaine de travail</button>'
          + '<button onclick="openPopup(\'popup-glossaire\')" style="width:100%;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:9px;padding:12px;font-size:0.78rem;color:#455A64;cursor:pointer;font-family:inherit;">📚 Glossaire juridique</button>'
      )
