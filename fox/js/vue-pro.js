...
(function () {
  ...
  function render() {
    ...
    var html = ''
      ...
      // ══ JOURNAL ════════════════════════════════════════════════
      + (journal.length ? section('Journal des analyses',
          '<div style="display:flex;flex-direction:column;gap:6px;">'
          + journal.map(function(j){
              return '<div style="background:rgba(255,255,255,0.02);border-radius:7px;padding:9px 11px;border-left:2px solid rgba(255,255,255,0.06);">'
                + '<div style="font-size:0.76rem;color:#546E7A;line-height:1.55;">' + (j.message||j.text||JSON.stringify(j)) + '</div>'
                + (j.date ? '<div style="font-size:0.6rem;color:#263238;margin-top:4px;">' + new Date(j.date).toLocaleDateString('fr-FR') + '</div>' : '')
                + '</div>';
            }).join('')
          + '</div>'
        ) : '')
      ...
      + section('Références légales',
          ...
          + [
            ...
            return '<div style="display:flex;gap:10px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.03);align-items:baseline;">'
              + '<div style="font-size:0.65rem;color:#37474F;min-width:72px;font-family:monospace;">Art. ' + r[0] + '</div>'
              + '<div style="font-size:0.73rem;color:#455A64;line-height:1.4;">' + r[1] + '</div>'
              + '</div>';
          }).join('')
          + '</div>'
      )
